...
ALGO_ORDER = ["REINFORCE", "SARSA", "DQN", "DDQN+PER", "A2C", "PPO", "SAC", "CrossQ"]
...
def plot(
    folders: str = typer.Option(
        ...
        "--folders",
        "-f",
        help="Comma-separated data folder names (e.g., ppo_cartpole_2026_01_11,a2c_gae_cartpole_2026_01_11)",
    ),
    title: str = typer.Option(
        None,
        "--title",
        "-t",
        help="Plot title. If omitted, extracted from spec env name.",
    ),
    legend_labels: str = typer.Option(
        None,
        "--legends",
        "-l",
        help="Comma-separated legend labels (must match number of folders)",
    ),
    data_folder: str = typer.Option(
        "data", "--data-folder", "-d", help="Base data folder path"
    ),
    output_folder: str = typer.Option(
        "docs/plots", "--output", "-o", help="Output folder for plots"
    ),
    showlegend: bool = typer.Option(
        True, "--legend/--no-legend", help="Show legend on plot"
    ),
):
    """
    ...
    custom_legends = (
        [label.strip() for label in legend_labels.split(",")] if legend_labels else None
    )
    if custom_legends and len(custom_legends) != len(folder_list):
        logger.error(
            f"Number of legend labels ({len(custom_legends)}) must match folders ({len(folder_list)})"
        )
        raise typer.Exit(1)
    ...
    for i, folder_name in enumerate(folder_list):
        ...
        if spec:
            algo_name = (
                get_algo_name_from_spec(spec)
                if not custom_legends
                else custom_legends[i]
            )
            ...
            algo_name = (
                custom_legends[i]
                if custom_legends
                else folder_name.split("_")[0].upper()
            )
    ...
    # Sort tracks by ALGO_ORDER for consistent ordering (skip when custom legends are provided)
    if not custom_legends:
        combined = []
    ...
    final_title = (
        title if title else (detected_title if detected_title else "Benchmark")
    )
...
def list_data(
    data_folder: str = typer.Option(
        "data", "--data-folder", "-d", help="Data folder path"
    ),
):
    """
