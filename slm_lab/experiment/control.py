...
class Session:
    ...
    def save_if_ckpt(self, agent, env):
        ...
        save_this_epi = env.done and epi != env.max_epi and epi > 0 and hasattr(env, 'save_epi_frequency') and epi % env.save_epi_frequency == 0
        if save_this_epi:
    ...
    def run(self):
        while self.env.clock.get('epi') < self.env.max_epi:
            self.run_episode()
