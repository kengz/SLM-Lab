...
def copy_spec(original_trial_path, new_trial_path):
    '''Copies spec from original to new directory. Used in eval mode.'''
...
def copy_models(original_session_name, new_trial_name, number_sessions):
    '''Copies all model data from original_session to new_trial. Duplicates model x number_sessions. Used in eval mode.'''
...
def prepare_eval_directory(new_spec, new_info_space, original_spec, original_info_space, original_prepath):
    '''Prepares a clean directory to evaluate a particular model. Leaves original experiment directory untouched.'''
