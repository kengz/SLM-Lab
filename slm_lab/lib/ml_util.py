"""ML utilities requiring heavy dependencies (numpy, torch, cv2)."""
...
class LabJsonEncoder:
    """JSON encoder that handles numpy types. Import and use as cls= in json.dumps."""
    import json
...
def set_cuda_id(spec):
    '''Use trial and session id to hash and modulo cuda device count for a cuda_id to maximize device usage.'''
