...
def check_comp_spec(comp_spec, comp_spec_format):
    ...
    for spec_k, spec_format_v in comp_spec_format.items():
        ...
        if ps.is_list(spec_format_v):
            v_set = spec_format_v
            assert comp_spec_v in v_set, (
                f"Component spec value {ps.pick(comp_spec, spec_k)} needs to be one of {util.to_json(v_set)}"
            )
        else:
            v_type = spec_format_v
            assert isinstance(comp_spec_v, v_type), (
                f"Component spec {ps.pick(comp_spec, spec_k)} needs to be of type: {v_type}"
            )
            if (
                isinstance(v_type, tuple)
                and int in v_type
                and isinstance(comp_spec_v, float)
            ):
...
def get(spec_file, spec_name, experiment_ts=None, sets: list[str] | None = None):
    ...
    # Read raw file, substitute variables, then parse
