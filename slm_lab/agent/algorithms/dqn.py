...
class DQN:
    ...
    batch_size: how many examples from memory to sample for each training step
    action_selection: function (from common.py). Determines how
    '''
    ...
    def compute_q_target_values(self, batch):
        ...
        q_targets_all = batch['rewards'] + self.gamma * (1 - batch['terminals'])  * self.net.eval(batch['next_states'])
        ...
        q_targets = torch.mul(q_targets_max, batch['actions']) + torch.mul(q_vals, (1 - batch['actions']))
        return q_targets
