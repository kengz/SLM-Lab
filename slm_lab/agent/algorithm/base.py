...
class Algorithm(ABC):
    ...
    def to_action(self, action: torch.Tensor) -> np.ndarray:
        ...
        # Vector continuous environments need (num_envs, action_dim) shape
            ...
            if (
                action_np.ndim == 1
            ):  # Got (num_envs*action_dim,), need (num_envs, action_dim)
                action_np = action_np.reshape(
                    self.agent.env.num_envs, self.agent.env.action_dim
                )
    ...
    def load(self):
        ...
        if not hasattr(self, "net_names"):
            logger.info(
                "No net declared in self.net_names in init_nets(); no models to load."
            )
        else:
