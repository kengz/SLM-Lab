...
# TODO change doc
class SPLearningBayesianPolicy(Algorithm):
    '''
    ...
    def init_nets(self, global_nets=None):
        ...
        if self.targets == "actions":
            ...
            raise NotImplementedError()
        out_dim = int(out_dim * 2)  # First half for mean (mu), second half for variance
    ...
    def proba_distrib_params(self, x, net=None):
        ...
        # soft min, soft max on logvar
        logvar = self.meta_algo.max_logvar - F.softplus(self.meta_algo.max_logvar - logvar)
        logvar = self.meta_algo.min_logvar + F.softplus(logvar - self.meta_algo.min_logvar)

        pdparam = torch.cat([mean, logvar], dim=0)
        return pdparam

    @lab_api
    def act(self, state):
        raise NotImplementedError()  # Used for planning only (in the LE use case)
    ...
    def supervised_learning_baysian_loss(self, batch, pdparams):
        ...
        half_len = int(len(preds) / 2)
        mean, logvar = preds[:half_len], preds[half_len:]
        ...
        # loss = self.net.loss_fn(preds, targets).mean()
        loss += 0.01 * (self.net.max_logvar.sum() - self.net.min_logvar.sum())
        loss += 1e-4 * (self.net.parameters**2).sum()


        self.to_log["loss_policy"] = loss
        ...
        #     self.to_log["entropy_over_loss"] = (entropy_loss / loss + self.episilon).clamp(
        #             min=-100, max=100)
        #     loss += entropy_loss
    ...
    def train(self, loss_penalty=None):
        ...
        if self.to_train == 1:
            ...
            loss = self.supervised_learning_baysian_loss(batch, pdparams)
            ...
            self.net.train_step(loss, self.optim, self.lr_scheduler, clock=self.internal_clock, global_net=self.global_net)
